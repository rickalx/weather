...
let loadDayForecastData = (b) => {
  if (b != undefined && b != null) {
    ...
    var { lapse, text, temperature, forecast, icon } = forecast_today[0];
    ...
    var { lapse, text, temperature, forecast, icon } = forecast_today[1];
...
let loadWeekForecastData = (a) => {
  ...
  for (let item of a.forecast_week) {
    let { day, text, date, temperature, icon } = item;
...
addEventListener("DOMContentLoaded", (event) => {
  ...
  btnCargar.addEventListener("click", (e) => {
    loadWeekForecastData(auxCiudades);
  });
});
